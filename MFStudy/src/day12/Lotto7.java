	...
	public static void main(String[] args) {
		...
		firstNum = lotto7.createFirstNum(args);// 1등번호 추가
		 if (set.getCountRank().length>firstNum.size()) {
				...
				System.exit(0);;
		}else if (validation.checkSetRankCount(set.getCountRank().length+1,firstNum.size())==false) {//등수 총 수가 로또 한게임 총수보다 큰지 판단 -크다-false 작다-true
			System.out.println("종료합니다.");
			System.exit(0);
		}
		...
		System.out.println(numberForRank.size());
		/* 이제 만들어야 랭크별 갯수 구하기 */
		for (int i = 0; i < numberForRank.size(); i++) {
